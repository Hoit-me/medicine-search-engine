...
import { REDIST_STREAM } from './constant';
import {
  RedisInstance,
  ...
  implements CustomTransportStrategy
{
  ...
  async bindHandlers() {
    console.log('bindHandlers', this.messageHandlers.keys());
    try {
      await Promise.all(
  ...
  private async handleRepondBack({
    ...
    inboundContext: RedisStreamContext2;
    insDisposed: boolean;
  }) {
    try {
      if (!this.redis) return;
  ...
  private async publishResponses(
    ...
    if (typeof responses === 'object') {
      ...
      if (!responses.pattern) return;
      if (!this.client) return;
      const serializedEntries = await this.serializer.serialize(
        { ...responses.payload, id: '' }, // id: '' is a temporary fix
        inboundContext,
      );
      await this.client.xadd(responses.pattern, '*', ...serializedEntries);
      return true;
    }
    return true;
  }
  ...
  private async registerStream(pattern: string) {
    try {
      console.log('registerStream', pattern);

...
import { DUR_PREGNAT_API_URL_BUILD } from '@src/constant';
import { Dur } from '@src/type/dur';
import { from, map, mergeMap, toArray } from 'rxjs';
import { UtilProvider } from '../util.provider';

@Injectable()
export class DurPregnantTabooBatchService {
  constructor(
  ...
  batch$() {
    return this.util
      .fetchOpenApiPages$<{ item: Dur.Ingredient.Pregnant.OpenApiDto }>(
        DUR_PREGNAT_API_URL_BUILD,
        100,
        2,
        'ASC',
      )
  ...
  /// ------------------------------------
  bulkUpsert$(
    data: Prisma.dur_ingredient_pregnant_tabooCreateInput[],
    batchSize: number = 20,
  ) {
    return from(data).pipe(
      mergeMap((data) => {
        const { id, ...rest } = data;
        return this.prisma.dur_ingredient_pregnant_taboo.upsert({
          where: { id },
          create: data,
          update: rest,
        });
